...
from ista_net_plus_phi import ISTANetplus
...
log_file_name = "./%s/phi_layer_%d_group_%d_ratio_%d_lr_%.4f.txt" % (
...
model_dir = "./%s/phi_layer_%d_group_%d_ratio_%d_lr_%.4f" % (args.model_dir, layer_num, group_num, cs_ratio, learning_rate)
...
for epoch_i in range(start_epoch+1, end_epoch+1):
    ...
    for data in rand_loader:
        ...
        cnt += 1
        epoch_loss_acc += loss_all.item()
        epoch_loss_tot += loss_all.item()
        
        if cnt % 20 == 0:
            epoch_loss.append(epoch_loss_acc / 20); epoch_loss_acc = 0
            plt.plot(epoch_loss, 'r-'); plt.savefig('tmp/epoch.png')
    ...
    rcd_loss_epoch.append(epoch_loss_tot / cnt)
    plt.figure()
    plt.plot(rcd_loss_epoch, 'b-')
    plt.savefig('tmp/epoches.png')
    # plt.clf()
